...
let [editing, setediting]= useState(details);
    ...
    useEffect(()=>{  
     ...
     var response=await axios.get("https://inv-1.herokuapp.com/category/getcategory",{
      headers:{
        token:token
      }
      })
    ...
    const editbrand=(async(event,id)=>{
      event.preventDefault();
       await axios.patch(`https://inv-1.herokuapp.com/category/updatecategory/${id}`,
        {
        name:editing.name,
    ...
    const deletebrand=async(id)=>{
      await axios.delete(`https://inv-1.herokuapp.com/category/deletecategory/${id}`,{
        headers:{
          token:token
        }
      })
   ...
   const Addbrand=async(e)=>{
     e.preventDefault();
    await axios.post(`https://inv-1.herokuapp.com/category/postcategory`,
    {
    name:editing.name,
    status:editing.status,
    },{
