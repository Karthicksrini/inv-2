...
let [editing, setediting]= useState(details);
    ...
    useEffect(()=>{  
     ...
     var response=await axios.get("https://inv-1.herokuapp.com/size/getsize",{
      headers:{
        token:token
      }
    ...
    const editbrand=(async(event,id)=>{
      event.preventDefault();
       await axios.patch(`https://inv-1.herokuapp.com/size/updatesize/${id}`,
        {
        name:editing.name,
    ...
    const deletebrand=async(id)=>{
      await axios.delete(`https://inv-1.herokuapp.com/size/deletesize/${id}`,{
        headers:{
          token:token
        }
   ...
   const Addbrand=async(e)=>{
     e.preventDefault();
    await axios.post(`https://inv-1.herokuapp.com/size/postsize`,
    {
    name:editing.name,
    status:editing.status,
    },{
